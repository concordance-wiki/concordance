...
function harness(behaviour?: Behaviour, text = "some text") {
...
describe("convertToPdf", () => {
  it.each([".docx", ".pptx", ".xlsx"])(
    ...
    async (extension) => {
      ...
      expect(output).toEqual({
        representations: { pdf: { path: `${cacheDirectory}/convert/${helloSha}.pdf` } },
        findings: [],
      });
  ...
  it("addresses the cache by the SHA-256 fingerprint of the source file: no unchanged file is reconverted", async () => {
  ...
  it("does not extract text from the PDF of a small document, whose emptiness is not suspect", async () => {
    const { deps, extracted } = harness(undefined, "");
  ...
  it("keeps the conversion cache and its temporary files in the pipeline cache, never next to the source", async () => {
    ...
    const output = await convertToPdf({ path: "/repo/a.docx", bytes: hello }, options, {
      ...
      fs: spied,
      cacheDirectory,
      extractText: () => Promise.resolve("text"),
    });
    ...
    expect(fs.listFiles(cacheDirectory)).toEqual([`convert/${helloSha}.pdf`]);
