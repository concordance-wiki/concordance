...
A WSDL that declares no operation of its own and imports another document (`wsdl:import`, a deployment document holding the bindings and the service alone) is reported as unreadable, naming the document to point the note at instead. Candidate objects come from the messages of each operation: in 1.1 the `element` or `type` of every `part` of its `input`, `output` and `fault` messages; in 2.0 the `element` of its `input` and `output`. A referenced name declared by an inline `types/schema` (`xsd:element` or `xsd:complexType`) is walked: the `type`, `ref` and `base` references of its definition are followed, transitively and once each, into the other elements and complex types the schema declares. A name that no inline schema declares, one that an `xsd:import` brings in for instance, is kept by name: the imported schemas are not fetched, and the built-in XML Schema types (`string`, `int`, `dateTime`, ...) are never candidates. A simple type reached through a definition is not a candidate; one a part names directly is kept like any referenced name.
