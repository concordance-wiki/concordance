...
The first twenty mentions are in the served HTML (`build.mentions_inline`); the rest loads from a JSON fragment per entity. Never a global index. A page weighs under 150 KB excluding previews. Today the fragment of an entity holds its rendered note and, for a keyword page, its passages; the mentions of the served HTML come from the provenances of the links of the model, located in the note that cites the page, whichever end of the link relation typing put first.
