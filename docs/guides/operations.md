...
The build runs `git` for every git source, and the conversion plugin runs LibreOffice. A command is located by an absolute path, never by a name looked up on the `PATH` of the process: `git` is taken from the system directories of the platform (`/usr/bin`, `/usr/local/bin`, `/opt/homebrew/bin` on macOS; `C:\Program Files\Git\cmd` on Windows), so that a folder a build could write to never supplies a command. A machine that keeps git elsewhere names it through the `CONCORDANCE_GIT` environment variable; without a git in those places the build stops with an error that says so. The scripts of the repository follow the same rule for `git` and for `pnpm` (the pnpm running the script, else `PNPM_HOME`, else the corepack shipped with Node.js).
