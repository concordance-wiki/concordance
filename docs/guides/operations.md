...
`.concordance-cache/` next to `concordance.yaml` (`conversion.cache` moves it) holds the shallow clones of the git sources under `sources/<name>/`, updated on the next build, and will hold the converted documents under `convert/`, keyed by the SHA-256 of their source so that an unchanged document is never reconverted, even when it moves. Nothing in it is needed to publish; nothing in it is secret either, beyond what the sources themselves contain. Keep it out of the configuration repository (`.gitignore`) and inside the pipeline cache (the [pipeline examples](pipelines.md) do), and delete it to force fresh clones and a full reconversion. The linter keeps its own cache, the published model of the global scope, under `.concordance-cache/lint` of the knowledge repository.
