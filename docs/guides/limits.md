...
- The build is not incremental: every run recomputes the whole model from the sources; only the clones of the git sources (and, later, converted documents) are kept in the cache. [Operations](operations.md#what-to-expect) gives the durations.
- No office document is converted: `convert: true` is accepted, no converter runs, documents stay downloadable entities without preview or extracted text, and `build.fail_on.unconverted_max` never triggers.
- The readers of the plugins (VTT transcripts, office metadata) are loaded but not called by the pipeline: a transcript or an office file is not read, so nothing is pseudonymised yet; the configuration and the dictionary are validated all the same.
