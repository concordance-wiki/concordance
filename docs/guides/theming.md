...
| Slot | Props |
|---|---|
| `Shell` | `locale`, `direction` (`ltr` or `rtl`), `title`, `head: { inlineScripts?, stylesheets, modulePreloads, scripts, classicScripts?, favicon? }`, `children` |
...
| `SearchResults` | `query`, `total`, `results: { title, href, typeLabel?, breadcrumb?, snippet? }[]` (`breadcrumb`: the titles of the application and domain), `facets: { name, label, values: { value, count, href }[] }[]` |
