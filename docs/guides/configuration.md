...
| `type_prefixes` | language pack | words that announce a type (`screen`, `API`, `table`, `data object`), one or several, right before the mention with nothing but spaces between, and add 0.1 confidence; the defaults are those of the profile for the locale of the source |
