...
`legal` holds what only the organisation that publishes the site can declare, and nothing is ever pre-filled: `mentions_url`, `accessibility_url` and `privacy_url` are the HTTPS addresses of the legal notice, the accessibility statement and the personal data page, linked from the footer of every page under "Declared by the organisation"; without an address, a note filed at `legal/mentions.md`, `legal/accessibility.md` or `legal/privacy.md` in a source stands for the page and is linked instead; without either, the link is not shown. `accessibility_status`, one of `non-compliant`, `partially-compliant` or `compliant`, is the state the statement declares, worded after its link ("Accessibility — partially compliant"); without it the link reads "Accessibility" alone, since the tool cannot establish a state.
