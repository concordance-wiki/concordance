...
The URL of a page follows the identifier of its entity and nothing else, so it stays the same from one build to the next as long as the identifier does. The main content of every page is in the served HTML: the text of the note, the section headings, the neighbours and the first mentions read without JavaScript, which only adds the mode switch, the disclosure of the remaining mentions and the search. No preview of converted documents is written yet.
