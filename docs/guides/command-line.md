...
The index is a set of classic scripts, not JSON files fetched over HTTP, so that the search works when `dist/index.html` is opened from the disk: browsers refuse `fetch` and module scripts from a `file://` page but load a plain `<script src>`. `search/meta.js` holds the entity table (identifier, title, type, URL, application, domain, status, source, with the labels the results show); `search/<prefix>.js` holds the tokens starting with two given characters and the entities carrying each, with their weight. The page loads the table when the field takes focus and one shard per word typed, once. On the realistic corpus of the fixtures, the whole index weighs about 130 kB in 155 shards, the largest shard under 4 kB and the table about 60 kB.
