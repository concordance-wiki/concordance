...
The field in the header of every page searches the whole site as you type, without a server: the build writes an index under `dist/search/`, and the page loads it in pieces. Press `/` anywhere on a page to reach the field, `Escape` to leave it; the best results appear under the field, each a link with its title, the query marked in it, its type or the notice that no note defines the expression, and its space; `↓` walks down the rows and `↑` back up to the field, `Enter` opens the row in focus, and the last line leads to the results page, `search/index.html?q=…`, with the whole list, which follows the field too. The field at the head of the home page is the same field drawn large, its results in the flow of the page with the number of matches next to it; `/` reaches it first there.
