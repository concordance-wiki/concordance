...
|---|---|---|---|
| `reader` | `readers` | `extensions`, each starting with `.` | `read({ path, payload: { bytes } }) → { metadata, text, units? }` |
