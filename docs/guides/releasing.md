...
1. creates the npm organisation `concordance-wiki`, which holds the scope of the seventeen `@concordance-wiki/*` packages, and settles the name of the preset: the unscoped name `concordance` belongs to another package on npm, so the preset is renamed before the first publication (a scoped name, `@concordance-wiki/concordance`, is the natural one), a decision recorded in this guide, in the getting-started guide and in the container image when it is taken;
