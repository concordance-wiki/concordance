...
A recurring expression that no note defines is reported as [`W-TERM-UNDEFINED`](../checks/W-TERM-UNDEFINED.md) and gets a keyword page built from its passages. Two answers: write the term note, whose address takes over the keyword page, or, when the expression is not a term of the corpus, list it under `rejected_terms` of the [lock file](configuration.md#lock) next to the configuration; the decision holds from the next build, compared without regard to case or plural, and the expression leaves the findings, the to-do page and the marks in the text. The same file settles a [`W-DUP-CANDIDATE`](../checks/W-DUP-CANDIDATE.md) on two notes that only share a base name: a `separated` pair is never reported again, a `merged` one becomes a single page.
