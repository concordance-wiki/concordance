...
`.github/workflows/pages.yml` publishes the folder on GitHub Pages at the root of the project site on every push to `main`, with the component gallery of the default theme rendered under `gallery/` next to it. The page is kept here until the organisation site repository exists; it then moves there unchanged. `scripts/validate.mjs` checks that it stays a single well-formed document whose links are absolute.
