...
- Centre: the breadcrumb "Space › folder › page", the title, the line "type · changed N days ago · space", the note rendered section by section, then the footer "path/of/file.md — Something to correct? Edit this page", the path linked to the file on its forge when known, the edit link as before, else to `project.contribute_url`, and no call to action when neither is known.
