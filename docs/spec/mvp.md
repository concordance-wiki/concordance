...
| Official plugins | `@concordance-wiki/plugin-reader-vtt`, `/plugin-reader-office`, `/plugin-convert-libreoffice`, `/plugin-contract-openapi`, `/plugin-contract-wsdl`; the PDF viewer ships with the default theme of `@concordance-wiki/site`; the contract viewer is a UI component of the default theme, in `/site` |
...
- The page keeps working without JavaScript (the field of the bar is a plain `GET` form to the results page, which the island fills from the address, the whole table under the facets for an empty query, as before; served, the results page says in its status region that the search runs in the browser and needs JavaScript, never "0 results") and over `file://`; the state stays in the URL, the ✕ rewriting the current entry at once.
