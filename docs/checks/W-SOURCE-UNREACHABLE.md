...
The repository URL is wrong, the ref does not exist, the pipeline has no credentials for it, or the local path is missing. Nothing from that source enters the model: its entities are absent, and links towards them from other sources resolve to nothing until the source comes back.
