...
describe("scannableText", () => {
  describe("Excluded: fenced and indented code blocks, inline code, URLs, frontmatter, and link targets", () => {
    ...
    it("removes an autolink and a www address from the text", () => {
      expect(
        texts("See <https://example.test/contract> or www.example.test/contract now.\n"),
      ).toEqual(["See  or  now."]);
    });
    ...
    it("keeps the text of a link and drops its target", () => {
      expect(texts("Checked by the [annual cap](../rules/contract.md).\n")).toEqual([
        "Checked by the annual cap.",
      ]);
    });

    it("drops an image entirely, alternative text included", () => {
      expect(units("![contract](contract.png)\n")).toEqual([]);
      expect(texts("Shown as ![contract](contract.png) here.\n")).toEqual(["Shown as  here."]);
    });

    it("gives no unit for raw HTML and keeps the text around an inline tag", () => {
      expect(texts("<div>contract</div>\n\nA <b>bold</b> word.\n")).toEqual(["A bold word."]);
    });
    ...
    it("gives no unit containing a term that appears only in a code block", () => {
      const found = units(
        "# Entry\n\nThe amount is checked.\n\n```sh\nversement libre\n```\n\n    versement libre\n\nDone with `versement libre`.\n",
      );
      expect(found.some((unit) => unit.text.includes("versement libre"))).toBe(false);
  ...
  describe("The visible text of a markdown link remains subject to recognition", () => {
    it("keeps the visible text of a link at its line and section", () => {
      expect(units("## Objects\n\nReads: [contract](../objects/contract.md).\n")).toEqual([
        ...
        { line: 3, text: "Reads: contract.", kind: "paragraph", section: "Objects" },
      ]);
    });

    it("keeps the visible text of a reference link", () => {
      expect(texts("Reads: [contract][c].\n\n[c]: ../objects/contract.md\n")).toEqual([
        "Reads: contract.",
      ]);
    });
  });
  ...
  describe("units", () => {
    it("makes a heading of every depth a unit, the H1 with no section", () => {
      expect(units("# Free payment entry\n\n## Objects\n\n### Detail\n")).toEqual([
        { line: 1, text: "Free payment entry", kind: "heading" },
    ...
    it("makes every list item a unit, nested items separately, at their own lines", () => {
      expect(units("## Objects\n\n- Reads: contract\n  - member\n- Writes: payment\n")).toEqual([
        { line: 1, text: "Objects", kind: "heading", section: "Objects" },
        { line: 3, text: "Reads: contract", kind: "list-item", section: "Objects" },
        { line: 4, text: "member", kind: "list-item", section: "Objects" },
        { line: 5, text: "Writes: payment", kind: "list-item", section: "Objects" },
      ]);
    });
    ...
    it("makes the paragraphs of a block quote units of kind quote, its list items staying list items", () => {
      expect(units("> A quoted contract.\n>\n> - item\n>\n> ```\n> code\n> ```\n")).toEqual([
        { line: 1, text: "A quoted contract.", kind: "quote" },
    ...
    it("reads the paragraphs of a footnote definition", () => {
      expect(units("Text[^1].\n\n[^1]: The contract note.\n")).toEqual([
        ...
        { line: 3, text: "The contract note.", kind: "paragraph" },
      ]);
    });
