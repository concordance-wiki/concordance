...
describe("resolveTheme", () => {
  ...
  it("rejects a component for a name that is not a slot", async () => {
    ...
    await expect(
      ...
      new ThemeResolutionError(
        "plugin @example/theme, theme custom: Sidebar is not a slot; slots are Shell, Header, Footer, Home, EntityPage, KeywordPage, MentionsPanel, Neighbourhood, SearchResults, Index, Todo, Spaces, Space, EntityPage@<type>, Attribute@<attribute> or Section@<section>",
      ),
    );
  });
