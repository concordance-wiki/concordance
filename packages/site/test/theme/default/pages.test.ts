...
describe("Home", () => {
  it("renders the title, the statistics, the search field with shortcuts and the three entry points", () => {
    ...
    expect(html).toContain('<a class="chip" href="glossary/payment/">payment</a>');
...
describe("KeywordPage", () => {
  it("renders the banner, the three counts, the passages by file, the companions and the similar forms", () => {
    ...
    expect(html).toContain("<h1>annual cap</h1>");
    expect(html).toContain(
    ...
    expect(html).toContain(
      '<h3><a href="../record-a-payment/">processes/record-a-payment.md</a></h3>',
    );
    expect(html).toContain(
      '<a href="../record-a-payment/#L12">line 12</a><q>the annual cap applies</q>',
    );
    expect(html).toContain(
      '<li class="companion" data-weight="5"><a href="../payment/">payment</a></li>',
    );
    expect(html).toContain('<li class="companion" data-weight="2"><span>ceiling</span></li>');
    expect(html).toContain("Offered as a lead: nothing here asserts a relation.");
    expect(html).toContain('<a href="../annual-caps/">annual caps</a>');
    expectBalanced(html);
  });
...
describe("SearchResults", () => {
  it("renders the summary, the facets with counts and the ordered results", () => {
    ...
    expect(html).toContain('<p class="search-summary">2 results for <q>payment</q></p>');
    expect(html).toContain('<nav class="facets" aria-label="Facets">');
    expect(html).toContain(
      '<a href="?q=payment&amp;type=term">term <span class="count">1</span></a>',
    );
    expect(html).toContain('<ol class="results">');
    expect(html).toContain(
      '<span class="badge">term</span><p class="snippet">A payment made at the member\'s request.</p>',
    );
    expect(html).toContain('<a href="../processes/record-a-payment/">Record a payment</a></li>');
    expectBalanced(html);
  });
...
describe("Index", () => {
  it("renders active letters as links, the current one marked, inactive ones visibly disabled", () => {
    ...
    expect(html).toContain('<a class="letter" href="../index/a/" aria-current="page">A</a>');
    expect(html).toContain('<span class="letter inactive" aria-disabled="true">B</span>');
    expect(html).toContain('<a class="letter" href="../index/c/">C</a>');
    expectBalanced(html);
  });
  ...
  it("marks a word without a note and shows the glyph and count of the others", () => {
    const html = renderSlot("Index", index, defaultTheme);
    expect(html).toContain(
      '<span class="noteless">no note</span><a href="../keywords/annual-cap/">annual cap</a><span class="count">7</span>',
    );
    expect(html).toContain(
      '<span class="glyph" aria-hidden="true">T</span><a href="../glossary/application/">application</a>',
    );
  });
});
...
describe("Todo", () => {
  it("lists the documents without markdown and the words without a note, each with its count", () => {
    ...
    expect(html).toContain(
      '<a href="../contracts/terms/">contracts/terms.docx</a> <span class="count">3</span>',
    );
    expect(html).toContain(
