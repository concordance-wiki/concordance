 ...
 * the version, the import date, the operations as a plain list with the ones no note describes
 * yet flagged and counted, the download link) and the island that shows the rest on demand. The
 * markdown of the note is untouched: the contract is displayed, never copied into it.
 */
export function ContractSection(props: ContractSectionProps): JSX.Element {
  const undocumented = props.operations.filter((operation) => !operation.documented).length;
  return (
    <section class="contract" aria-labelledby="contract-title">
      ...
      <h3 id="contract-operations">
        ...
        {undocumented > 0 && (
          <span class="contract-gap">
            {undocumented} {labels.withoutNote}
          </span>
        )}
      </h3>
      {props.operations.length === 0 ? (
        ...
        <ul class="contract-operations" aria-labelledby="contract-operations">
          {props.operations.map((operation) => (
            <li
              key={operation.name}
              class={operation.documented ? "contract-documented" : "contract-undocumented"}
            >
              <a href={operation.href}>{operation.title}</a>
              {operation.summary !== undefined && (
                <span class="contract-summary"> {operation.summary}</span>
              )}
              {!operation.documented && <span class="contract-flag">{labels.noNoteYet}</span>}
            </li>
          ))}
        </ul>
      )}
