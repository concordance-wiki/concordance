...
export const galleryPages: readonly GalleryPage[] = [
  ...
  },
  {
    ...
    state: "api-corporate",
    description:
      "the model query API of the fixtures corpus in the corporate chrome: the tree of the space with the operations under the current interface, the breadcrumb, the line naming the type, the change and the space, the note, then the operations table matched to the contract by operation name, the method as a chip, the path, the title of the note and the callers, with two gap rows in italics, one present in the contract without a page and one described without existing in the contract; the contract block with its format, its file, its import date, its download link and the viewer behind its button; in the panel the five keys with their note, the related pages with the operations first, the neighbourhood folded",
    ...chrome,
    header: corporateHeader,
    footer: corporateFooter,
