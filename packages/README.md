...
| `ingest` | git clone with the whole history and without the blobs, cache, file reading, markdown and frontmatter parsing | core |
...
| `site` | slots and view models, default theme, islands and their bundles, stylesheet layers, page budget, search index, accessibility checker | core, i18n, profile |
| `ui` | reserved for client components; nothing exported yet, the islands live in `site` | — |
| `cli` | `build`, `export`, `gallery`, `init`, `lint`, `mcp`, `query`, `render`, `validate-config` | all |
